class Game {
	constructor() {
		...
		options.assets.push(`${game.assetsPath}fbx/salon.fbx`);
	...
	loadEnvironment(loader) {
		const game = this;
		loader.load(`${this.assetsPath}fbx/salon.fbx`, function (object) {
			game.environment = object;
			game.colliders = [];
			game.scene.add(object);
			const textureLoader = new THREE.TextureLoader();

			textureLoader.load(`https://dm0qx8t0i9gc9.cloudfront.net/thumbnails/image/rDtN98Qoishumwih/wood-tile-texture-background_HP9bf1d3fg_thumb.jpg`, function (texture) {
	...
	constructor(game, options) {
		...
		if (options === undefined) {
			...
			if(people.includes(url.searchParams.get("modal"))){
				model = url.searchParams.get("modal")
			}else{
				model= 'BusinessMan'
			}
		...
		loader.load(`${game.assetsPath}fbx/people/${model}.fbx`, function (object) {
			...
			player.object.position.set(-600, 0, 1073);
			player.object.rotation.set(0, 1.6, 0);
